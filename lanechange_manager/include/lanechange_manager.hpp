...
public:
    ...
    CalcAroundWaypoints(){
        ROS_ERROR("Not Configurated ros::Rate param for CalcAroundWaypoints");
        exit(1);
    }
    ~ CalcAroundWaypoints(){}
    CalcAroundWaypoints(ros::Rate* change_rate)
    : cur_vel(0.0)
    , prev_x(0.0)
    , prev_y(0.0)
    , lane_x(0.0)
    ...
    , local_rate(change_rate)
    {
        ...
        sub_global_path = nh.subscribe("/global_lane_waypoints_array", 10,  &CalcAroundWaypoints::callbackGlobalPath, this);
        ...
        alive_pub = nh.advertise<std_msgs::String>("lanechange_check", 10);
    ...
    double near_lane_x = 0.0, near_lane_y = 0.0;
