...
bool CalcAroundWaypoints::checkOverlapLanes()
{
    int overlap_count = 0;

    for(auto& lane : global_lane.lanes)
    {
    ...
    if(overlap_count > 1)
    {
        // ROS_INFO("Lanes in current pose are overlaped!");
        if(!overlap_published)
        {
            manager_state.tryNextState("lanes_are_overlaped");
            publishFirstCurrentLane(global_lane);
            overlap_published = true;
        }
        wait = setWaitTime(5);
        return true;
    }
    ...
    {
        // ROS_INFO("Lanes are not overlaped!");
        manager_state.tryNextState("lanes_are_not_overlaped");
        overlap_published = false;
        return false;
    }
}
/////////////////////////////////
// check function session end. //
/////////////////////////////////
...
void CalcAroundWaypoints::run()
{
    ...
    int cur_index;
    //ROS_ERROR("change_done : %d", change_done);
    if(change_done)
    {
        //ROS_ERROR("2");
        onSign("straight");
